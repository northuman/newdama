...
//		}
		...
		static void OnSetStory () {
			...
			BindMissingExternalFunctions(_story);
		...
		static void BindMissingExternalFunctions (Story story) {
			var externalNames = new HashSet<string>();
			CollectExternalFunctionNames(story.mainContentContainer, externalNames);
			foreach (var name in externalNames)
				story.BindExternalFunctionGeneral(name, (args) => null, lookaheadSafe: true);
		}

		static void CollectExternalFunctionNames (Container container, HashSet<string> names) {
			foreach (var content in container.content) {
				var divert = content as Divert;
				if (divert != null && divert.isExternal)
					names.Add(divert.targetPathString);
				var sub = content as Container;
				if (sub != null)
					CollectExternalFunctionNames(sub, names);
			}
			foreach (var kvp in container.namedContent) {
				var sub = kvp.Value as Container;
				if (sub != null)
					CollectExternalFunctionNames(sub, names);
			}
		}
